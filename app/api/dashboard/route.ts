...
export async function GET() {
  ...
  const checkpointCompanyFilter = user?.role === "SUPER_ADMIN" ? "" : "JOIN condominiums c ON c.id = cp.condominium_id AND c.company_id = $1";

  const [todayPatrols, completedPatrols, delayedPatrols, incidents, activeGuards, checkpoints, weekly, activities, alerts] =
    await Promise.all([
      ...
      query(`SELECT count(*)::int AS total FROM checkpoints cp ${checkpointCompanyFilter}`, companyParams),
      query(
        `SELECT to_char(day, 'DD/MM') AS label, COALESCE(count(p.id),0)::int AS total
         ...
         LEFT JOIN patrols p ON p.created_at::date = day::date
           ${user?.role === "SUPER_ADMIN" ? "" : "AND EXISTS (SELECT 1 FROM condominiums c WHERE c.id = p.condominium_id AND c.company_id = $1)"}
         GROUP BY day
         ORDER BY day`,
        companyParams
      ),
  ...
  return Response.json({
    kpis: {
      ...
      checkpoints: checkpoints.rows[0].total
    },

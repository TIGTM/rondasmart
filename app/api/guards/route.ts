...
export async function GET() {
  ...
  const result = await query(
    `SELECT u.id, u.name, u.email, u.role, u.phone, u.registration, u.shift, u.status, u.company_id, u.condominium_id,
            c.name AS condominium_name
     FROM users u
     LEFT JOIN condominiums c ON c.id = u.condominium_id
     WHERE u.role IN ('GUARD', 'MANAGER')
       ${user?.role === "SUPER_ADMIN" ? "" : "AND u.company_id = $1"}
     ORDER BY CASE u.role WHEN 'MANAGER' THEN 0 ELSE 1 END, u.name`,
    params
  );
...
export async function POST(request: Request) {
  ...
  const role = body.role === "MANAGER" ? "MANAGER" : "GUARD";
  ...
  const result = await query(
    `INSERT INTO users (company_id, name, email, password_hash, role, phone, registration, shift, status, condominium_id)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,COALESCE($9,'Disponivel'),$10)
     RETURNING id, company_id, name, email, role, phone, registration, shift, status, condominium_id`,
    [
      companyId,
      body.name,
      ...
      role === "MANAGER" ? "Supervisor" : body.shift ?? null,
      body.status ?? "Disponivel",
      role === "MANAGER" ? body.condominiumId ?? null : body.condominiumId ?? null
    ]
  );
